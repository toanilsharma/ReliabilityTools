...
const MtbfCalculator: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (h) setTotalHours(h);
    if (f) setFailures(f);
    
    if (h && f && !isNaN(parseFloat(h)) && !isNaN(parseFloat(f))) {
        setResult(calculateMTBF(parseFloat(h), parseFloat(f)));
    }
  }, [location.search]);
    ...
    <div className="grid md:grid-cols-2 gap-8">
      <div className="space-y-6">
        ...
        {result !== null && (
          <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl p-6 relative group">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-700 pt-2 mt-2">
              Failure Rate (λ): <strong>{(1 / result).toExponential(4)}</strong>{" "}
              failures/hour
            </div>
          </div>
        )}
        ...
        {result !== null && (
            <ShareResult 
                title="MTBF/MTTF" 
                params={{ hours: totalHours, failures: failures }} 
            />
        )}
      </div>
  ...
  const Content = (
    <div>
      ...
      <p>
        <strong>Mean Time Between Failures (MTBF)</strong> and{" "}
        <strong>Mean Time To Failure (MTTF)</strong> are the two most critical
        Key Performance Indicators (KPIs) in reliability engineering. They
        provide a data-driven baseline for predicting system availability and
        planning maintenance schedules.
      </p>

      <div className="grid md:grid-cols-2 gap-6 my-8">
        <div className="bg-cyan-50 dark:bg-cyan-900/10 p-6 rounded-xl border border-cyan-100 dark:border-cyan-800/30">
          <h3 className="text-lg font-bold text-cyan-800 dark:text-cyan-300 mb-2">
            MTBF (Repairable)
          </h3>
          <p className="text-sm text-cyan-800/80 dark:text-cyan-200/70">
            Used for assets that can be fixed, such as pumps, motors, and
            conveyor belts. It measures the average time between one breakdown
            and the next.
          </p>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/10 p-6 rounded-xl border border-purple-100 dark:border-purple-800/30">
          ...
          <p className="text-sm text-purple-800/80 dark:text-purple-200/70">
            Used for disposable items like light bulbs, fuses, or O-rings. It
            measures the average lifespan before the item must be replaced.
          </p>
        </div>
      </div>
      ...
      <p>
        The calculation is straightforward but often misunderstood. It is simply
        the ratio of total operating time to the number of failures.
      </p>
      <p>
      ...
      <ul>
        <li>
          You have a fleet of <strong>10 pumps</strong>.
        </li>
        <li>
          They run for <strong>24 hours</strong> a day for{" "}
          <strong>10 days</strong>.
        </li>
        <li>
          Total Operational Time = 10 pumps * 24 hours * 10 days ={" "}
          <strong>2,400 hours</strong>.
        </li>
        <li>
          During this time, you experience <strong>4 breakdowns</strong>.
        </li>
      </ul>
      <p>
      ...
      <p>
        MTBF is a core component of the Inherent Availability formula, along
      ...
      <p>
        To increase availability, you must either increase reliability (MTBF) or
        decrease downtime (MTTR). World-class organizations focus on both.
      </p>

      <h2 id="bathtub">The Bathtub Curve</h2>
      <p>
        It is vital to assume that MTBF applies to the "Useful Life" phase of
        the Bathtub Curve, where the failure rate is constant.
      </p>
      <ul>
        <li>
          <strong>Infant Mortality:</strong> High failure rate at startup
          (installation errors).
        </li>
        <li>
          <strong>Useful Life:</strong> Constant, low failure rate (random
          events).
        </li>
        <li>
          <strong>Wear Out:</strong> Increasing failure rate (aging).
        </li>
      </ul>
      <p>
        MTBF is the inverse of the constant failure rate (&lambda;) during the
        useful life phase. <code>MTBF = 1 / &lambda;</code>.
      </p>
    </div>
  );
